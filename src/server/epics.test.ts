...
const { epicFinished, findDependencyCycle, hasChildren, heldByEpicOrder, listChildren, unmetDependencies } =
  await import("./epics");
...
describe("epics", () => {
  ...
  it("holds a graph epic's piece while a dependency is unfinished, and not otherwise", () => {
    seed("epic", { runMode: "graph" });
    seed("a", { parentCardId: "epic", status: "done", position: 1 });
    seed("b", { parentCardId: "epic", status: "looping", position: 2 });
    seed("c", { parentCardId: "epic", status: "todo", position: 3, dependsOn: ["a"] });
    seed("d", { parentCardId: "epic", status: "todo", position: 4, dependsOn: ["a", "b"] });
    seed("e", { parentCardId: "epic", status: "todo", position: 5, dependsOn: null });

    // c's only dependency is done; d still waits on b; e depends on nothing.
    expect(heldByEpicOrder(card("c"))).toBe(false);
    expect(heldByEpicOrder(card("d"))).toBe(true);
    expect(heldByEpicOrder(card("e"))).toBe(false);
    expect(unmetDependencies(card("d"), listChildren("epic")).map((row) => row.id)).toEqual(["b"]);

    // Abandoned counts as finished for the graph, like done.
    db.update(cards).set({ status: "abandoned" }).where(eq(cards.id, "b")).run();
    expect(heldByEpicOrder(card("d"))).toBe(false);

    // A dependency id that matches no sibling does not hold the piece.
    seed("f", { parentCardId: "epic", status: "todo", position: 6, dependsOn: ["ghost"] });
    expect(heldByEpicOrder(card("f"))).toBe(false);
  });

  it("finds a cycle in a breakdown's dependency graph", () => {
    expect(findDependencyCycle([{}, { dependsOn: [0] }, { dependsOn: [1] }])).toBeNull();

    const cycle = findDependencyCycle([{ dependsOn: [2] }, { dependsOn: [0] }, { dependsOn: [1] }]);
    expect(cycle).not.toBeNull();
    expect(cycle![0]).toBe(cycle![cycle!.length - 1]);
    expect(cycle).toEqual(expect.arrayContaining([0, 1, 2]));

    expect(findDependencyCycle([{ dependsOn: [0] }])).toEqual([0, 0]);
  });
