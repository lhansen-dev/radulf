...
/** Radulf's agent env (agentEnv in src/server/harness/types.ts) sets GIT_SSH_COMMAND=/bin/false for every sandboxed run, and nothing on a plain dev host or CI does. Inside that sandbox a nested sandbox cannot start, so the real-runtime suites below skip themselves with this as the signal.
 *
